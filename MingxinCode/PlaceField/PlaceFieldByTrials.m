function map = PlaceFieldByTrials(RawPositionInfo,SmoothedPositionInfo,FiringInfoPath,RawOccmap,SmoothedOccmap,RewardPts,varargin)
...
for a = 1:length(FiringFiles)
    ...
    plot(raw_txy.p(start:stop,3),raw_txy.p(start:stop,2));
...
%     savefig(fullfile('PlaceFields_New/FullAnalysis',[FigName '_full_analysis']));
%     saveas(gcf,fullfile('PlaceFields_New/FullAnalysis/FullAnalysis_png',[FigName '_full_analysis']),'png');
end
...
map.Frequency = imgaussfilt(map.Frequency,1);
map.name = filename;
