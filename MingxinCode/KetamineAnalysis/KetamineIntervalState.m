function KetamineIntervalState(basepath,basename)
...
% Brendon Watson 2016, used for basic intervals (by states) detection,
% Mingxin Ding 2017
...
% injp4start = InjectionComparisionIntervals.InjPlus2HourEndRecordingSeconds;
injp4end = InjectionComparisionIntervals.InjPlus4HourRecordingSeconds;
...
injp4int = intervalSet(injp2end,injp4end); % 2-4 hour after injection
...
if ~isempty(BadIntervali)
   ...
   injp4int = minus(injp4int,BadIntervali);
end
...
int1WAKE = intersect(baseint,WAKEInts);
int2WAKE = intersect(base24int,WAKEInts);
int1MA = intersect(baseint,MAInts);
int2MA = intersect(base24int,MAInts);
int1NREM = intersect(baseint,NREMInts);
int2NREM = intersect(base24int,NREMInts);
int1REM = intersect(baseint,REMInts);
int2REM = intersect(base24int,REMInts);

baselineStateIntervals = v2struct(int1WAKE,int1MA,int1NREM,int1REM);
baseline24StateIntervals = v2struct(int2WAKE,int2MA,int2NREM,int2REM);


int1WAKE = intersect(injm1int,WAKEInts);
int2WAKE = intersect(injp1int,WAKEInts);
int1MA = intersect(injm1int,MAInts);
int2MA = intersect(injp1int,MAInts);
int1NREM = intersect(injm1int,NREMInts);
int2NREM = intersect(injp1int,NREMInts);
int1REM = intersect(injm1int,REMInts);
int2REM = intersect(injp1int,REMInts);

InjM1StateIntervals = v2struct(int1WAKE,int1MA,int1NREM,int1REM);
InjP1StateIntervals = v2struct(int2WAKE,int2MA,int2NREM,int2REM);


int2WAKE = intersect(injp2int,WAKEInts);
int2MA = intersect(injp2int,MAInts);
int2NREM = intersect(injp2int,NREMInts);
int2REM = intersect(injp2int,REMInts);

InjP2StateIntervals = v2struct(int2WAKE,int2MA,int2NREM,int2REM);

int2WAKE = intersect(injp4int,WAKEInts);
int2MA = intersect(injp4int,MAInts);
int2NREM = intersect(injp4int,NREMInts);
int2REM = intersect(injp4int,REMInts);

InjP4StateIntervals = v2struct(int2WAKE,int2MA,int2NREM,int2REM);
...
KetamineIntervalState = v2struct(...
    baselineStateIntervals,baseline24StateIntervals,...
    InjM1StateIntervals,InjP1StateIntervals,...
    InjP2StateIntervals,InjP4StateIntervals,...
    baseint,base24int,injm1int,injp1int,injp2int,injp2int);

save(fullfile(basepath,[basename '_KetamineIntervalState.mat']),'KetamineIntervalState');
