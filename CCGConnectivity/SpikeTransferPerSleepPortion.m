function SpikeTransferPerSleepPortion = SpikeTransferPerSleepPortion(basepath,basename,numdivisions)
...
for a  = 1:length(WakeSleep)
    ...
    %into proportions.
    int = intersect(subset(WakeSleep{a},2),SWSPacketInts);
...
SpikeTransferPerSleepPortion = v2struct(EPairs,IPairs,ConnectionStarts,ConnectionEnds,Flips,...
    bintimes,normtimes,...
...
save(fullfile(basepath,[basename '_SpikeTransferPerSleepPortion.mat']),'SpikeTransferPerSleepPortion')
...
function [SlopesAbs,SlopesNorm,medianSlopesAbs,medianSlopesNorm] = localgetslopes(numWS,Pairs,datamtx,bintimes,normtimes)
SlopesAbs = [];
SlopesNorm = [];
for a = numWS
