function funcsynapses = ReDo_FindSynapse(funcsynapses,S,shank,cellIx)

%% Parameters
alpha = funcsynapses.AlphaValue;%signifiance cutoff/pvalue
BinSize = funcsynapses.BinMs;%in ms
synapticwindow = funcsynapses.SynapticWindowInMs;%Milliseconds in which synapses much occur (inclusive)...
                        % = synWin input to FindSynapses.m
convolutionwidth = funcsynapses.ConvolutionWidthInBins; % size of the convolution window (in number of bins
                        % = convWin input to FindSynapses.m
excSigWin = funcsynapses.ExcMinimumWidth; % duration of actual ccg above threshold for exc. synapses
inhSigWin = funcsynapses.InhMinimumWidth; % duration of actual ccg below threshold for inh. synapses
