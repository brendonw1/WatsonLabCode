...
elseif nargin<5 || isempty(axh)    %Did the user specify a handle?
    % If the user didn't specify a handle, use gca to get a handle to the current axis
    axh = gca;
end
...
switch state
    case 'on'
        ...
        %% Get the axes data series line plots. lineh is NOT the cursor lines (or anything generated by dualcursor) it's the original data.
        lineh = local_findDataSeriesLines(axh);
        ...
        if isempty(lineh)
            ...
            %if multiple data series lines on the axes, define callback to allow for selection
...
%           % Gets the first dataseries line and sets it as the "SelectedLine"
        ...
        yl = ylim(axh); % get the local y-axes maximum value
        ...
        % Set up mouse cursor behavior for lines:
        pointerBehavior.enterFcn =@(hfig, cpp)set(hfig, 'Pointer', 'left');
        pointerBehavior.traverseFcn = [];
        pointerBehavior.exitFcn = @(hfig, cpp)set(hfig, 'Pointer', 'arrow');
        iptSetPointerBehavior(ph1, pointerBehavior);
        iptSetPointerBehavior(ph2, pointerBehavior);

        % Set up mouse cursor behavior for text boxes:
        pointerBehavior_text.enterFcn =@(hfig, cpp)set(hfig, 'Pointer', 'fleur');
        pointerBehavior_text.traverseFcn = [];
        pointerBehavior_text.exitFcn = @(hfig, cpp)set(hfig, 'Pointer', 'arrow');
        iptSetPointerBehavior(th1, pointerBehavior_text);
        iptSetPointerBehavior(th2, pointerBehavior_text);
...
%         cmenu = uicontextmenu('Parent',get(axh,'Parent'));
        ...
        %Store handle to dataseries line and cursor handle for text boxes:
        set(th1,'UserData',[lineh ph1]);     
        set(th2,'UserData',[lineh ph2]);
    ...
    case 'down'      % Execute the WindowButtonDownFcn
        % "gco" gets the handle of the last interacted-with object.
        htype = get(gco,'Type');
        tag = get(gco,'Tag');
...
%             set(gco,'EraseMode','xor')
        ...
        if strcmp(tag,'Cursor') %If clicked on a cursor
            ...
            % TODO: Could display the x-value or point index here instead.
    ...
    case 'move'          % Execute the WindowButtonMotionFcn
        ...
        if ~isempty(gco)
            ...
            if strcmp(tag,'CursorText')             %The text
                ...
                ph = handles(2); % Get the actual cursorline from the handles.
                slide = 0;      %Don't slide along line; just reposition text
            else                                    %The marker
                ph = gco;
                handles = get(gco,'UserData');
                th = handles(2); % Get the actual cursorline from the handles.
...
%             set(gco,'EraseMode','Normal')
            ...
            if strcmp(tag,'Cursor')        %Delete the temporary cursor number label (because it goes away when the mouse is up)
                CNh = getappdata(gco,'CNh');
                delete(CNh);
            end
    ...
    case 'selectline'          % User selected a new data series line to be active
    ...
    case 'update'               %Update the cursor value
        ...
        % Get data from the selected line
        xl = get(lh,'XData');
        yl = get(lh,'YData');
    ...
    case 'off'   % Unset the WindowButton...Fcns
        ...
        h3 = findobj(axh,'Tag','CursorDeltaText');           %The cursors delta text
    ...
    case 'onlyOneCursor' % Sets the visibility to hidden for the second data cursor:
        %Find the existing cursors
        cursors = findobj(axh,'Tag','Cursor');
        cn1 = getappdata(cursors(1),'CursorNumber');
        cn2 = getappdata(cursors(2),'CursorNumber');
        ...
        %% Set the second datacursor objects not visible:
        cursors(cn2).Visible = 'off';
        th2.Visible = 'off';   
        % deltah(1) is the x delta, and deltah(2) is the y. Neither are relevant when the second cursor is hidden.
        deltah(1).Visible = 'off';
        deltah(2).Visible = 'off';
        
    case 'showSecondCursor' % Renable the visibility of the second data cursor:
        %Find the existing cursors
        cursors = findobj(axh,'Tag','Cursor');
        cn1 = getappdata(cursors(1),'CursorNumber');
        cn2 = getappdata(cursors(2),'CursorNumber');
        ...
        %% Set the second datacursor objects not visible:
        cursors(cn2).Visible = 'on';
        th2.Visible = 'on';   
        % deltah(1) is the x delta, and deltah(2) is the y.
        deltah(1).Visible = 'on';
        deltah(2).Visible = 'on';
...
% Tries to find data series (line plots) on the axes provided. Does NOT get data cursor lines.
function lineh = local_findDataSeriesLines(axh)
lineh = findobj(axh,'Type','line');        %Find a line (like a line plot on the axis, not a cursor line generated by this program) to add cursor to
dots = findobj(axh,'Type','line','Tag','Cursor');  %Get existing cursors
lineh = setdiff(lineh,dots); % returns the data in lineh that is not in dots, with no repetitions. This ignores existing cursors.
...
linehtemp = lineh;
lineh=[];
if ~iscell(xdtemp)      %If there's only one plot line, force data into a cell array
    xdtemp = {xdtemp};
end
% For each line plot "series"
for ii=1:length(xdtemp)
    % Get series lines with a length (of data points) longer than 2 elements
...
% Returns x and y mins and maxes for all children dataseries plots on the axes
