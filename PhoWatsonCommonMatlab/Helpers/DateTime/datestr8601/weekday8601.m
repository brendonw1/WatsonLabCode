...
% Supports English names only, can easily be adapted for other languages.
%
%%% Syntax:
% weekday8601()
...
if nargin==0
	...
	assert(isnumeric(dtn)&&isreal(dtn),'First input <t> must be a real numeric array.')
	assert(all(isfinite(dtn(:))),'First input <t> must contain finite values only.')
end
%
if nargin>1&&ischar(fmt)&&isrow(fmt)
	[isf,idf] = ismember(lower(fmt),{'short','long'});
	assert(isf,'Second input <fmt> must be either ''short'' or ''long''.')
else
	idf = 1+(nargin>1&&fmt);
end
%
