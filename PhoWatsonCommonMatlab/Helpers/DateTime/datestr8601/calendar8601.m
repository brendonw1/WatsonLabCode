...
%
%%% Inputs:
%  y = NumericScalar, a valid Serial Date Number.
%    = CharacterRow,  a valid Date String.
%    = NumericVector, a valid Date Vector.
%    = Datetime, a scalar datetime variable.
...
elseif nargin==1 && isnumeric(y)
	if numel(y)>1 % datevec:
		y = datenummx(y);
		assert(isscalar(y),er1,'y')
	end % datenum:
	[y,m,~] = datevecmx(y);
elseif nargin==2 && isnumeric(y) % year & month:
	m = varargin{1};
	assert(erf(y,-Inf,Inf),er1,'y')
	assert(erf(m,   0, 13),er2,'m')
else % datestr | datetime:
	[y,m,~] = datevec(y,varargin{:});
	assert(isscalar(y),er1,'y')
	assert(isscalar(m),er2,'m')
end
%
% Generate Calendar
%
...
if nargout
	...
	ism = dtv(1)==y && dtv(2)==m;
	mat = permute(cat(3,mat,32+10*((mat==dtv(3))&ism)),[3,1,2]);
