function episodes = GetTripletSleepEpisodes(intervals,REMmin,SWSmin,REMmerge,SWSmerge,restrictinginterval)
...
% 		thispreSWS = shift(subset(SWSintervals,SWSRightBefore),offset);
%         thisREM = shift(subset(REMintervals,a),offset);
%         thispostSWS = shift(subset(SWSintervals,SWSRightAfter),offset);
        thispreSWS = subset(SWSintervals,SWSRightBefore);
        thisREM = subset(REMintervals,a);
        thispostSWS = subset(SWSintervals,SWSRightAfter);

        episodes{end+1} = cat(thispreSWS,thisREM,thispostSWS);
	end
end
1;
