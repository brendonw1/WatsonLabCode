function SleepPETHs_OFFDursIntsOverPacketsSWSEpsSleep
warning off
...
for a = 1:length(dirs);
    ...
    uptimes = mean([starts ends],2);
    updurs = ends-starts;
    upints = starts(2:end)-ends(1:end-1);    
    timesD = uptimes;
...
%% saving and plotting
OFFDursIntsOverSleepInts = v2struct(normbool,normmode,normperiod,sffororiginalepochs,binspernormepoch,statenames,...
    AllEpochsD,AllEpAlignedD,AllEpochsI,AllEpAlignedI);

h = [];
for b = 1:numstatetypes
    h(end+1) = PlotPETHFigure(AllEpAlignedD{b},params);
    set(h(end),'name',['OFFDurationsOver' statenames{b}])
    h(end+1) = PlotPETHFigure(AllEpAlignedI{b},params);
    set(h(end),'name',['OFFIntervalsOver' statenames{b}])
end

MakeDirSaveVarThere('/mnt/brendon4/Dropbox/BW OUTPUT/SleepProject/PETHS/UPs/',OFFDursIntsOverSleepInts)
