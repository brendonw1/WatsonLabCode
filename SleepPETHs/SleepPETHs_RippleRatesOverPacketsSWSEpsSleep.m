function SleepPETHs_RippleRatesOverPacketsSWSEpsSleep
warning off
...
sffororiginalepochs = 1;%samples per second
binspernormepoch = 20;
...
for a = 1:length(dirs);
    ...
    load(fullfile(basepath,'Ripples',[basename '_RippleSpikeStats.mat']))
...
%% saving and plotting
RippleRatesOverSleepInts = v2struct(normbool,normmode,normperiod,sffororiginalepochs,binspernormepoch,statenames,...
    AllEpochsE,AllEpAlignedE,AllEpochsI,AllEpAlignedI);

h = [];
for b = 1:numstatetypes
    h(end+1) = PlotPETHFigure(AllEpAlignedE{b},params);
    set(h(end),'name',['ECellRippleRatesOver' statenames{b}])
    h(end+1) = PlotPETHFigure(AllEpAlignedI{b},params);
    set(h(end),'name',['ICellRippleRatesOver' statenames{b}])
end

MakeDirSaveVarThere('/mnt/brendon4/Dropbox/BW OUTPUT/SleepProject/PETHS/Ripples/',RippleRatesOverSleepInts)
