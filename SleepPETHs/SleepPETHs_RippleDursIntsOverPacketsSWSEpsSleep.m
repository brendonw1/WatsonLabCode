function SleepPETHs_RippleDursIntsOverPacketsSWSEpsSleep
warning off
...
for a = 1:length(dirs);
    ...
    for b = 1:numstatetypes
        ...
        [Epochs,~,EpAligned] = IntervalPETH_Irregular(dataD,timesD,stateInts{b},binspernormepoch,sffororiginalepochs);
        if normbool
        ...
        [Epochs,~,EpAligned] = IntervalPETH_Irregular(dataI,timesI,stateInts{b},binspernormepoch,sffororiginalepochs);
        if normbool
            [EpAligned,Epochs] = NormPETHToPrePost(EpAligned,Epochs);
        end
...
%% saving and plotting
RippleDursIntsOverSleepInts = v2struct(normbool,normmode,normperiod,sffororiginalepochs,binspernormepoch,statenames,...
    AllEpochsD,AllEpAlignedD,AllEpochsI,AllEpAlignedI);

h = [];
for b = 1:numstatetypes
    h(end+1) = PlotPETHFigure(AllEpAlignedD{b},params);
    set(h(end),'name',['RippleDurationsOver' statenames{b}])
    h(end+1) = PlotPETHFigure(AllEpAlignedI{b},params);
    set(h(end),'name',['RippleIntervalsOver' statenames{b}])
end

MakeDirSaveVarThere('/mnt/brendon4/Dropbox/BW OUTPUT/SleepProject/PETHS/Ripples/',RippleDursIntsOverSleepInts)
