function SleepPETHs_SpindleIncidenceOverPacketsSWSEpsSleep
warning off
...
for a = 1:length(dirs);
    ...
    for b = 1:numstatetypes
        ...
        % I Population 
        if sum(isnan(dataI)) < numel(dataI)%unless there are no I cells;
...
%% saving and plotting
SpindleIncidenceOverSleepInts = v2struct(normbool,normmode,normperiod,sffororiginalepochs,binspernormepoch,statenames,...
...
%     set(h(end),'name',['UPDurationsOver' statenames{b}])
    ...
    set(h(end),'name',['SpindleIncidenceOver' statenames{b}])
end

MakeDirSaveVarThere('/mnt/brendon4/Dropbox/BW OUTPUT/SleepProject/PETHS/Spindles/',SpindleIncidenceOverSleepInts)
