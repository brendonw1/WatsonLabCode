function SleepPETHs_PopRateOverPacketsSWSEpsSleep
warning off
...
for a = 1:length(dirs);
    ...
    load(fullfile(basepath,[basename '_PopRatePerBinData.mat']))
    ...
    times = PopRatePerBinData.bincentertimes;
    dataE = PopRatePerBinData.EPopRatePerCell;
    dataI = PopRatePerBinData.IPopRatePerCell;
...
%             tdataI(~status) = nan;%setting to nan works because in eventual mean/plot we use nanmean.  
        ...
        if ~isempty(stateInts{b})
            [Epochs,~,EpAligned] = IntervalPETH(tdataE,stateInts{b},numnormbins,sf,params,mask);
    %         [Epochs,~,EpAligned] = IntervalPETHg(tdataE,times,stateInts{b},binspernormepoch,sffororiginalepochs);
            ...
            if sum(isnan(tdataI)) < numel(tdataI)%unless there are no I cells;
                [Epochs,~,EpAligned] = IntervalPETH(tdataI,stateInts{b},numnormbins,sf,params,mask);
...
%% saving and plotting
PopRateOverSleepInts = v2struct(normbool,normmode,normperiod,sffororiginalepochs,statenames,...
    AllEpochsE,AllEpAlignedE,AllEpochsI,AllEpAlignedI);

h = [];
for b = 1:numstatetypes
    h(end+1) = PlotPETHFigure(AllEpAlignedE{b},params);
    set(h(end),'name',['ECellRateOver' statenames{b}])
    h(end+1) = PlotPETHFigure(AllEpAlignedI{b},params);
    set(h(end),'name',['ICellRateOver' statenames{b}])
end

MakeDirSaveVarThere('/mnt/brendon4/Dropbox/BW OUTPUT/SleepProject/PETHS/PopRate/',PopRateOverSleepInts)
MakeDirSaveFigsThereAs('/mnt/brendon4/Dropbox/BW OUTPUT/SleepProject/PETHS/PopRate/',h)
MakeDirSaveFigsThereAs('/mnt/brendon4/Dropbox/BW OUTPUT/SleepProject/PETHS/PopRate/',h,'png')
...
%     sf = 1/mode(diff(PopRatePerBinData.bincentertimes));
% %     parms.prepostsec = [-20 40];
% 
%     % E Population Coefficient of Variation
%     dataE = PopRatePerBinData.CoVE;
