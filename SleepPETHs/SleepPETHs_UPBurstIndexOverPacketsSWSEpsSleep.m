function SleepPETHs_UPBurstIndexOverPacketsSWSEpsSleep
warning off
...
% gathering data which won't change depending on the interval selected:
    ...
    dataE = UPONBurst.UMBIE;
    dataI = UPONBurst.UMBII;
...
for b = 1:numstatetypes
    ...
    set(h(end),'name',['ECellUPBurstIndexOver' statenames{b}])
    h(end+1) = PlotPETHFigure(AllEpAlignedI{b},params);
    set(h(end),'name',['ICellUPBurstIndexOver' statenames{b}])
end

MakeDirSaveVarThere('/mnt/brendon4/Dropbox/BW OUTPUT/SleepProject/PETHS/UPs/',UPEIBurstIndexOverSleepInts)
