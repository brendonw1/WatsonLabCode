function SleepPETHs_SpindleAmpFreqOverPacketsSWSEpsSleep
warning off
...
for a = 1:length(dirs);
    ...
    dataF = SpindleData.data.peakFrequency;
    dataA = SpindleData.data.peakAmplitude;
    ...
    for b = 1:numstatetypes
        ...
        [Epochs,~,EpAligned] = IntervalPETH_Irregular(tdataA,times,stateInts{b},binspernormepoch,sffororiginalepochs);
        if normbool
            [EpAligned,Epochs] = NormPETHToPrePost(EpAligned,Epochs);
        end
...
%% saving and plotting
SpindleAmpFreqOverSleepInts = v2struct(normbool,normmode,normperiod,sffororiginalepochs,binspernormepoch,statenames,...
    AllEpochsF,AllEpAlignedF,AllEpochsA,AllEpAlignedA);

h = [];
for b = 1:numstatetypes
    h(end+1) = PlotPETHFigure(AllEpAlignedF{b},params);
    set(h(end),'name',['SpindleFrequencyOver' statenames{b}])
    h(end+1) = PlotPETHFigure(AllEpAlignedA{b},params);
    set(h(end),'name',['SpindleAmplitudeOver' statenames{b}])
end

MakeDirSaveVarThere('/mnt/brendon4/Dropbox/BW OUTPUT/SleepProject/PETHS/Spindles/',SpindleAmpFreqOverSleepInts)
