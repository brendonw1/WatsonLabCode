function SleepPETHs_BurstAtTransitions
warning off
...
sffororiginalepochs = 0.1;%samples per second
binspernormepoch = 6;
...
%         end
        if sum(isnan(tdata)) < numel(tdata)%unless there are no I cells;
            ...
            [epochs,PETH] = TransitionPETH(tdata,preint,postint,[30 30],sf);
%             [Epochs,~,EpAligned] = IntervalPETH(tdata,times,stateInts{b},binspernormepoch,sffororiginalepochs);
...
%% saving and plotting
EIRatioOverSleepInts = v2struct(sffororiginalepochs,binspernormepoch,statenames,...
    AllEpochs,AllPETHs);
...
for b = 1:size(statecombonumbers,1);
    ...
    xlabel('sec')
    set(h(end),'name',['BurstOver' statecombonames{b}])
end

h(end+1) = TransitionPETHComparisonFigure(PETHTimes,AllPETHs);
xlabel('sec')
legend(statecombonames)
set(h(end),'name','BurstComparisonAcrossTransitions')
