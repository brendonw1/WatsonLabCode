...
for v = 1:length(u);
    if u(v).isdir 
%     if u(v).isdir & length(u(v).name)>length(animalname)
%         if strcmp(u(v).name(1:length(animalname)),animalname)
            ...
            if exist(fullfile(dirpath,[basename,'.dat']),'file')
                ...
                for a = 1:length(d)
                    if d(a).isdir 
                        if length(d(a).name) >= length(basename)
                            if strcmp(lower(d(a).name(1:length(basename)-2)),lower(basename(1:end-2)))
            ...
            %                         datpaths{end+1} = fullfile(dirpath,d(a).name,'amplifier.dat');
                ...
                t = ['! rm ' fullfile(dirpath, '/*/amplifier.dat')];
                try
                    eval(t)
                end
