function [h,r,p,coeffs,prepostpercentchg,postvpreproportion] = PlotPrePost_StableOnly(pre,post,smoothwidth)
...
prepostpercentchg = ConditionedPercentChange_StableOnly(pre,post);

% proportions of original - use geometric stats here
postvpreproportion = ConditionedProportion_StableOnly(pre,post);
...
if sum(pre<0)<=(length(pre)/2)%if most values positive
    subplot(2,1,1)
    ...
    xlabel('log10(pre)')
    ylabel('log10(post)')
else %if most values negative
    subplot(2,1,1)
    ScatterWStats(-pre,-post)
    xlabel('negative pre')
    ylabel('negative post')
    subplot(2,1,2)
    ScatterWStats(-pre,-post,'loglog')
    xlabel('log10(negative pre)')
    ylabel('log10(negative post)')
end 
...
%         hax3 = plot_nanmedianSD_bars(postvpreproportion(decreasers),postvpreproportion(ncs),postvpreproportion(increasers));
        hax4 = plot_stats_bars(postvpreproportion(decreasers),postvpreproportion(ncs),postvpreproportion(increasers),'central','nanmean','variance','sem');
        axes(hax4)
        hold on;
        plot([0 4],[1 1],'k')
        set(hax4,'Yscale','log')
...
%     hax = plot_nanmedianSD_bars(log10(postvpreproportion(q_rnk==1)),log10(postvpreproportion(q_rnk==2)),log10(postvpreproportion(q_rnk==3)),log10(postvpreproportion(q_rnk==4)));
    v1 = postvpreproportion(q_rnk==1);
    v2 = postvpreproportion(q_rnk==2);
    v3 = postvpreproportion(q_rnk==3);
    v4 = postvpreproportion(q_rnk==4);
%     v1 = real(log10(postvpreproportion(q_rnk==1)));
%     v2 = real(log10(postvpreproportion(q_rnk==2)));
%     v3 = real(log10(postvpreproportion(q_rnk==3)));
%     v4 = real(log10(postvpreproportion(q_rnk==4)));
