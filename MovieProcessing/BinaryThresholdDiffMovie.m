function threshdiffmovie = BinaryThresholdDiffMovie(diffmovie,opensize)
...
% 3D movie.  
% opensize specifies the size looked for in bwareaopen (number of
% contiguous pixels)

if ~exist('opensize','var')
    opensize = 15;
end
...
for a = 1:size(diffmovie,4);
    ...
    threshdiffmovie(:,:,a) = bwareaopen(tempimage,opensize);
%     if rem(a,500)==0
%         disp(['Frame #',num2str(a),' done']);
%     end
end
