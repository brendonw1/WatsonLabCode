function RippleDetectWrapper_hemisphere2(basepath,basename)
...
rip_ch = bmd.Ripplechannel_hemisphere2;
if isfield(bmd,'RippleNoiseChannel');
    noise_ch = bmd.RippleNoiseChannel_hemisphere2;
else
...
[~,eeglfpname] = fileparts(eeglfppath);
if isempty(noise_ch)
    lfp = LoadLfp(eeglfpname,num_ch,rip_ch);    
    lfp = [Range(lfp, 's') double(Data(lfp))];
    noiselfp = [];
else
    lfp = LoadLfp(eeglfpname,num_ch,[rip_ch noise_ch]);    
    lfp = [Range(lfp, 's') double(Data(lfp))];
...
%% Export final data
RippleData_hemisphere2 = v2struct(ripples,rip_ch,maps,data,stats);
% MakeDirSaveVarThere(fullfile(basepath),RippleData);
save(fullfile(basepath,[basename '_Ripples_hemisphere2.mat']),'RippleData_hemisphere2')
