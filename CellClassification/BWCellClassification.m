...
function  [CellClassOutput, PyrBoundary,Waveforms] = BWCellClassification (basepath,basename, cellshanknums, shanks, ESynapseCells, ISynapseCells)
...
if ~exist('ESynapseCells','var')
    ESynapseCells = [];
end
if ~exist('ISynapseCells','var')
    ISynapseCells = [];
end
...
%     theseChannels = Par.SpkGrps(thisshank).Channels;
    ...
    spkname = fullfile(basepath,[basename '.spk.' num2str(thisshank)]);
    
    cellsthisshank = cellnums(shanks==thisshank);
    intrashankclunums = cellshanknums(shanks == thisshank);
    cluname = fullfile(basepath,[basename '.clu.' num2str(thisshank)]);
    clu = LoadClu(cluname);
...
TroughPeakMs = x;
SpikeWidthMs = y;
MeanWaveforms = v2struct(AllWaves,MaxWaves,cellnums,cellshanknums,shanks,basename,TroughPeakMs,SpikeWidthMs,ELike,MaxWaveChannelIdxsB0);
