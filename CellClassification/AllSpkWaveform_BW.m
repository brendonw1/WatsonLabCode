function AllSpkWaveform_BW(basename)
...
%% Loading some data from par file... not in shige's original file.  But not bothering to change his original variable names
Par = LoadPar([basename '.xml']);
nChan = Par.nChannels;
Nshank = size(Par.SpkGrps,2);
FileN = [basename '.']; %basename

%% Scaling from Eran Stark's spike_stats.m
p2pvoltage = Par.VoltageRange;
scalefactor = 1 / 2^Par.nBits * p2pvoltage * 1e6 / Par.Amplification; % a2d units -> microVolts

%%
w = round(Par.SampleRate * 0.003); % # of samples in window around spike, using 3ms sample (w=60@20kHz)

%%
ij = 1;

for i_shank = 1:Nshank
    ...
    if size(spk_wf,1)>=2                             %% This part is the only deferrence from Alfonso's
      for n =2:size(spk_wf,1) % loop over clusters   %% original code. clu==0,1 is excluded here.
          SpkWvform_all(ij,:,:) = spk_wf( n, :,:)*scalefactor;   %% saving and also scaling to be in terms of voltage
...
save([FileN(1:end-1) '_SpikeWavsAll.mat'],'SpkWvform_all', 'SpkWvform_idx');
% save([FileN(1:end-1) '_SpikeWavsAll_TEST.mat'],'SpkWvform_all', 'SpkWvform_idx');
