function Spectrograms = GenerateSpectrograms(basepath,ChannelsOfInterest,plotting)
...
if ~exist('ChannelsOfInterest','var')
    ChannelsOfInterest = 1;%channels of interest, first channel is 0 (not 1)...like from neuroscope
end
...
filename = fullfile(basepath,[basename '.Spectrograms.mat']);

%% Check for prior spectrograms
if exist(filename,'file')
    load(filename);
    for a = 1:length(Spectrograms)
        chans(a) = Spectrograms{a}.info.Ch;
    end
    if prod(size(ChannelsOfInterest')) == prod(size(chans'))
        if ~any(sort(ChannelsOfInterest') - sort(chans'))%if no differences in list    
            disp('Loading prior spectrogram file - which had these same channels')
            return
        end
    end
end
...
nch = sessionInfo.nChannels;
samprate = sessionInfo.lfpSampleRate;

% %% Self explanatory
% ChannelsIWantSpectrogramsFor = [1:16];%channels of interest, first channel is 0 (not 1)...like from neuroscope

%% Get spectrograms
lfp = bz_LoadBinary(fullfile(basepath,[basename '.lfp']), 'channels', ChannelsOfInterest+1, 'frequency', samprate, 'nChannels', nch);%load in the .lfp file channel voltage
lfp = single(lfp);
for cidx = 1:length(ChannelsOfInterest);
   thischan = ChannelsOfInterest(cidx);
   
%    for i = 1:length(ChannelsOfInterest)
        disp(['Whitening and computing spectrogram for channel ', int2str(ChannelsOfInterest(cidx)),'. This will all be over in a minute.']);
        Spectrograms{cidx} =[];

        weeg{cidx} =  WhitenSignalIn(lfp(:,cidx)',samprate*2000,1);
        [Spectrograms{cidx}.spec, Spectrograms{cidx}.fo, Spectrograms{cidx}.to] = mtchglongIn(weeg{cidx}, 3072, samprate, samprate, 0, [], [], [], [0 200]);
        Spectrograms{cidx}.spec = single(Spectrograms{cidx}.spec);
        Spectrograms{cidx}.info.Ch = ChannelsOfInterest(cidx);
        Spectrograms{cidx}.info.FileInfo.name = [basename, '.lfp'];
%     end
    disp('Done.');
end
...
%save data to disk
save(filename,'Spectrograms')

%% For plotting
if plotting
    for cidx = 1:length(ChannelsOfInterest)
        figure;
        imagesc('XData',Spectrograms{cidx}.to,'YData',Spectrograms{cidx}.fo,'CData',log(Spectrograms{cidx}.spec)');
        axis xy tight
        axis tight
        xlabel('seconds')
        ylabel('Hz')
...
for j=1:nwin
    if ~isempty(ARmodel) 
        A = ARmodel;
        for ii=1:nCh
            y(seg(j,1):seg(j,2),ii) = Filter0In(A, x(seg(j,1):seg(j,2),ii));
        end
    else
        if CommonAR % meaning common model for all channels and segments!!! 
            for ii=1:nCh
                if  j==1 & ii==1
                    switch artype
                        case 1
                            [w Atmp] = arfitIn(x(seg(j,1):seg(j,2),ii),ArOrder,ArOrder);
                            A = [1 -Atmp];
                        case 2
                            A = arburg(x(seg(j,1):seg(j,2),ii),ArOrder);
                    end
                    ARmodel = A;
                end
                y(seg(j,1):seg(j,2),ii) = Filter0In(A, x(seg(j,1):seg(j,2),ii));
...
for ind=1:nDefArgs
    ...
    if (ind>nInArgs | isempty(Args{ind}))
        varargout(ind) = {DefArgs{ind}};
    else 
        varargout(ind) = {Args{ind}};
    end
end
...
for p = pmax:-1:pmin
    ...
    if p < pmax
        ...
        logdp(ix) = logdp(ix+1) + 2.* log(abs(prod(diag(L))));
    end
    
    % Schwarz's Bayesian Criterion
    sbc(ix) = logdp(ix)/m - log(ne) * (ne-np(ix))/ne;
    
    % logarithm of Akaike's Final Prediction Error
    fpe(ix) = logdp(ix)/m - log(ne*(ne-np(ix))/(ne+np(ix)));
    
    % Modified Schwarz criterion (MSC):
    % msc(i) = logdp(i)/m - (log(ne) - 2.5) * (1 - 2.5*np(i)/(ne-np(i)));
    
    ix      = ix-1;                % go to next lower order
end
