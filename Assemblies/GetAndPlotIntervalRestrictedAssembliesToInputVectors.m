function [AssemblyBasicData,h] = GetAndPlotIntervalRestrictedAssembliesToInputVectors(S,restrictints,CellIDs,intervals,binsecs,InputBinnedMatrix)
% specify interval of times to determine components from
% Plot that activity against specifically input matrix of rate vectors
% (InputBinnedMatrix)
...
AssemblyActivities = assembly_activity(Patterns,InputBinnedMatrix');
for a = 1:size(AssemblyActivities,1);
    aidxs = find(assemblynum==a);
    AssemblyCells{a} = cellnum(aidxs);
    AssemblyCellActivites(:,a) = sum(InputBinnedMatrix(:,AssemblyCells{a}),2);
...
for a = 1:size(AssemblyActivities,1);
    ...
    xlim([1 size(InputBinnedMatrix,1)])
    yl = get(gca,'ylim');
    ...
    subplot(3,1,3);
    plot(sum(InputBinnedMatrix,2),'k')
    xlim([1 size(InputBinnedMatrix,1)])
    title(['All Cells'])
end
