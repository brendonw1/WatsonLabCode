function [hax,Hbar, Herr] = plot_geomeanoflogofratiosSD_bars(varargin)
...
% Hbar: handle for bar graph output
...
for a = 1:length(datatoplot)
    ...
    if std(data) == 0
        SDs(a) = 0;%otherwise geometric mean gives SD=1 for distributions with no variance... for now don't want that
    else
        SDs(a) = geostd(data,[],1);
    end
end
