function HAx = plot_nanmeanSEM_bars(varargin)
...
for a = 1:length(datatoplot)
    ...
    medians(a) = nanmedian(data,1);
    SEMs(a) = nanstd(data,0,1)/length(data);
end
...
if sum(medians>0) == length(medians)%if all positive, set error bars up
    Herr = errorbar(xvals,medians,zeros(size(xvals)),SEMs,'Color','k','Marker','none','LineStyle','none');
elseif sum(medians<0) == length(medians)%if all negative, set error bars down
    Herr = errorbar(xvals,medians,SEMs,zeros(size(xvals)),'Color','k','Marker','none','LineStyle','none');
else %If pos & neg, do double error bars
    Herr = errorbar(xvals,medians,SEMs,SEMs,'Color','k','Marker','none','LineStyle','none');
end 
