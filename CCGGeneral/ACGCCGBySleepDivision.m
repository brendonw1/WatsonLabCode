function ACGCCGsBySleepDiv = ACGCCGBySleepDivision(basepath,basename,NumSleepDivisions)
...
DivFileName = fullfile(basepath,[basename '_SleepDivisions' num2str(NumSleepDivisions) '.mat']);
if ~exist(DivFileName,'file')
    GatherSleepDivisionIntervals(basepath,basename,NumSleepDivisions);
end
load(DivFileName);
...
ACGCCGsBySleepDiv = v2struct(slnames,NumSleepDiv,NumWS,CCGHalfWidthsinMs,numHalfBins);
    ...
    for a = 1:NumSleepDiv;
        ...
        if isempty(T)% if no spikes in interval:
            for b = 1:length(CCGHalfWidthsinMs)
                ...
                CCGs{a,b} = nan(length(Times{b}),numcells,numcells);
                ACGs{a,b} = nan(1+2*numHalfBins(b),numcells);
                ISIH{a,b} = nan(1+numHalfBins(b),numcells);
            ...
            for b = 1:length(CCGHalfWidthsinMs)
                ...
                if length(unique(G))<size(S,1)%account for a cell not firing in a state
                    ...
                    CCGs{a,b} = fillinmissingccg(CCGs{a,b},missing);
                end
                for c = 1:size(CCGs{a,b},2)
                    ACGs{a,b}(:,c) = CCGs{a,b}(:,c,c); 
                end
                [ISIH{a,b},ISIHTimes{b}] = ISIHistogram(thisS,HalfWidthInS(b),BinSizeInS(b));
                ISIH{a,b} = ISIH{a,b}'*1000;
            end
            if size(ACGs{a,b},2) ~= size(ISIH{a,b},2)
                1;
            end
