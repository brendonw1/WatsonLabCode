function [normtotalcounts_struct, AvailCells_struct, rawcounts_struct, befaftvals_struct, alldiffsPs_struct, allSpikesAnatomies_struct] = GatherIntervalsetTriggeredSpikingNormalized...
...
for tidx = 1:length(prestarts)
% for c = 1:length(prestarts);        %go through each interval
%     tidx = intervalidxs(c);%gather info
    triggerInterval = subset(intervalset,tidx);
...
befaftvals_struct = v2struct(befaftvals,Ebefaftvals,Ibefaftvals);
alldiffsPs_struct = v2struct(diffsPs,EdiffsPs,IdiffsPs);
allSpikesAnatomies_struct = v2struct(SpikesAnatomies,ESpikesAnatomies,ISpikesAnatomies);
