...
% for i=1:length(tsd)
	...
	if l
	  ...
	  for j=1:length(st)
		  ...
		  while spk(ix)<st(j) && ix<l %skip ahead if outside bin.. until find equal to or greater than start of bin...
			  ix = ix+1;
		  end
		  while spk(ix)<=en(j) && ix<l %then add + 1 to count in bin and move to next
			  dQ(j,i) = dQ(j,i)+1;
			  ix = ix+1;
		  end
		  if ix==l && spk(ix)<=en(j) && spk(ix)>st(j) %if at end but still in bin, count it (special case)
			  dQ(j,i) = dQ(j,i)+1;
          end
      end
	end
