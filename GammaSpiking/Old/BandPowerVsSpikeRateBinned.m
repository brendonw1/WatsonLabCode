function BandPowerVsSpikeRateBinned(basepath,basename)
% Spike rates of E/I/Sextiles vs various bands at 1Hz
% Old
...
%% ploting by bands
for a = 1:size(bands,1)

    gammaidxs = (fo>bands(a,1) & fo<=bands(a,2));
...
for a = numspikerategroups:-1:1
    plot(mean(bands,2),r_SextilesEWake(:,a))
end
...
hold on
for a = numspikerategroups:-1:1
    plot(mean(bands,2),r_SextilesEREM(:,a))
end
legend({'Highest FR','2','3','4','5','Lowest FR'},'Location','NorthEastOutside')
